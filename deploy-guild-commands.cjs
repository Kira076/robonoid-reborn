...
const { loadCommands } = require('./src/utils/command-handler.ts');

dotenv.config();

const commands = async () => {await loadCommands();};
...
(async () => {
    try {
        console.log(`Started refreshing ${commandData.length} commands and menus.`);
